...
// Create schema with FTS5 for full-text search over transcript and summary
...
export function updateItemSummaryAndTranscript(id, transcript, summary, keyPointsJson) {
  const stmt = db.prepare(`
    UPDATE items SET transcript=?, summary=?, key_points=? WHERE id=?
  `);
  stmt.run(transcript, summary, keyPointsJson, id);
}
